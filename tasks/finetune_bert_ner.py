...
def doEval(data_dir, max_seq_length, label_list, tokenizer, model, eval_batch_size=1):
    ...
    for input_ids, input_mask, label_ids in tqdm(eval_dataloader, desc="Evaluating"):
        ...
        with torch.no_grad():
            logits = model(input_ids, token_type_ids=None, attention_mask=input_mask, labels=label_ids)[1]
        ...
        for i, label in enumerate(label_ids):
            ...
            for j,m in enumerate(label):
                if j == 0:
                    ...
                    # handling <unk> as we using spm, just convert to O
                    try:
...
if finetune:
    ...
    model_name = 'epoch_54-bert_id_wikicombinedAll_basehead_lcase_uni50k_id'
    model_eval_name = 'epoch_0_0-bert_id_wikicombinedAll_tokenclasshead_lcase_uni50k_id_id'
    do_lower_case=True

    eval_only = True # set True to bypass training, or for doing evaluation only
    
    start_iters = 0
    from_pretrained = False # set True only in begining of process finetuning start.
    ...
    if eval_only:
        ...
        for eph in range(start_iters, int(num_train_epochs)):
            ...
            if eph % 1 == 0:
                ...
                _path = os.path.join('../../temporary_before_move_to_git/id-pytorch-transformers/samples/wiki_datasets/trained_model/bert/', 'epoch_{}-{}_id.ckpt'.format(eph, train_model_name))
